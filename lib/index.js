...
function paletteXImgFromEntry(entry, palette) {
    ...
    for (let y = 0; y < entry.lines.length; y++) {
        for (let x = 0; x < entry.lines[y].data.length; x++){
            const boi = entry.lines[y].data[x];
            const xA = entry.lines[y].unk80 + (entry.lines[y].data.length - x);
            const yA = (entry.lines.length-1 - y);
            const index = (xA + yA * W) * 4;
            const color = palette.data.colors[boi.color || 0] || 0;
            data[index] = color.r; // RED
            data[index + 1] = color.g; // GREEN
            data[index + 2] = color.b; // BLUE
            data[index + 3] = 255; // ALPHA
